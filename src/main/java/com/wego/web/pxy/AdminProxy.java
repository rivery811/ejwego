...
//       uid.add(uids);
	 ...
	 public String makeaddr() {
	        List<String> addr = Arrays.asList("동작구","서초구","구로구","서대문구");
	               Collections.shuffle(addr);
	               
	               String addrs = addr.get(0) ;
	              return addrs;
	    }
	 ...
	 public String pos() {
	        List<String> addr = Arrays.asList("대리","사원","팀장");
	        Collections.shuffle(addr);
	               String addrs = addr.get(0) ;
	              return addrs;
	    }
	 public String dep() {
	        List<String> addr = Arrays.asList("마케팅","영업","재무");
	        Collections.shuffle(addr);
	               String addrs = addr.get(0) ;
	              return addrs;
	    }
	    public String privilege() {
	        List<String>privilegeText = Arrays.asList("Y","N");
	        Collections.shuffle(privilegeText);
	        String privilege = privilegeText.get(0);
	        return privilege;
	    }
	    
	    public Admin makeAdmin(){
	        // //eid,pwd,aname,addr,tel,pos,dep,privilege;
	       return new Admin(makeUserid(),"1",makeUsername(),makeaddr(),makeTelephone(), pos(),dep(),privilege() );
	   }
	    
	    @Transactional
	    public void insertUsers() {
	 	   for(int i=0;i< 5;i++) {
	 		  adminMapper.insertAdminEid(makeAdmin());
	        }
